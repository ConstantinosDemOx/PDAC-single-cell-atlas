# B/Plasma cells gene programme discovery
...
FeaturePlot(lymphoid, features = "IL7R")
...
B.Pl <- subset(lymphoid, subset = Lvl2_Clusters %in% c("B"))
B.Pl

B.Pl.hq <- subset(B.Pl, subset = CD3E < 1 & CD3D < 1)


B.Pl.list <- SplitObject(B.Pl.hq, split.by = "Study")

# normalize and identify variable features for each dataset independently
B.Pl.list <- lapply(X = B.Pl.list, FUN = function(x) {
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 5000)
})

# select features that are repeatedly variable across datasets for integration
consensus_features <- SelectIntegrationFeatures(object.list = B.Pl.list, nfeatures = 5000)
...
counts <- GetAssayData(B.Pl.hq, assay = "RNA")
dim(counts)
...
B.Pl.hq.subset <- subset(B.Pl.hq, features = rownames(counts))

print(B.Pl.hq)
print(B.Pl.hq.subset)

rm(B.Pl.list);rm(counts);gc()
...
gp_generation <- function(x) {
  ...
  for (sample in cell_type_samples) {
    ...
    x.s <- x.s %>% 
      NormalizeData() %>% FindVariableFeatures(nfeatures = 500) %>% ScaleData() %>% RunPCA()
...
gp_generation(B.Pl.hq.subset)

write.csv(gene.programmes, "Consensus method/Figure 2/Assets/B/gene.programmes.csv")
write.csv(jaccard.matrix, "Consensus method/Figure 2/Assets/B/jaccard.matrix.csv")


gene.programmes <- read.csv("Consensus method/Figure 2/Assets/B/gene.programmes.csv", row.names = 1)
jaccard.matrix <- read.csv("Consensus method/Figure 2/Assets/B/jaccard.matrix.csv", row.names = 1)
...
fviz_nbclust(jaccard.matrix, FUN = kmeans, method = "wss")
...
rcc = ConsensusClusterPlus(as.matrix(jaccard.matrix),maxK=10,reps=500,pItem=0.8,pFeature=1,
                           ...
                           title = "Consensus method/Figure 2/Assets/B/ccp_results",
                           distance="pearson",clusterAlg="pam")

saveRDS(rcc, "Consensus method/Figure 2/Assets/B/B.Pl.consensus.results.rds")
rcc <- readRDS("Consensus method/Figure 2/Assets/B/B.Pl.consensus.results.rds")
...
B.Pl_gps <- split(x = fc_res.top$gene, f = fc_res.top$GP)
...
B.Pl_gps <- lapply(B.Pl_gps, top_20_genes)
B.Pl_gps

optK <- 6

saveRDS(B.Pl_gps, "Consensus method/Figure 2/Assets/B/B.Pl_gps.2.rds")

#optK <-5

B.Pl_gps <- readRDS("Consensus method/Figure 2/Assets/B/B.Pl_gps.rds")
B.Pl_gps
# Apply to NK cells


colnames(B.Pl.hq@meta.data)
B.Pl.hq@meta.data <- B.Pl.hq@meta.data[,-c(18:length(colnames(B.Pl.hq@meta.data)))]

B.Pl.hq <- AddModuleScore(B.Pl.hq, features = B.Pl_gps, name = "GP")


B.Pl.gp_mat <- B.Pl.hq@meta.data %>% 
  select(names(B.Pl_gps)) %>% scale() %>% as.data.frame()


assigned_GPs <- apply(B.Pl.gp_mat, 1, function(row) {
...
B.Pl.gp_mat$GP <- assigned_GPs


table(B.Pl.gp_mat$GP)
colnames(B.Pl.gp_mat)

B.Pl.gp_mat <- B.Pl.gp_mat %>% rownames_to_column(var = "Cell.ID")
max_values <- apply(B.Pl.gp_mat[,2:(optK+1)], 1, max)
table(B.Pl.gp_mat$GP)

B.Pl.gp_mat$Max_Variable <- max_values

cell.mat.top2000 <- B.Pl.gp_mat %>%
  group_by(GP) %>% 
...
cell.mat.split <- split(x = B.Pl.gp_mat$Cell.ID, B.Pl.gp_mat$GP)


saveRDS(cell.mat.top2000, "Consensus method/Figure 2/Assets/B/cell.mat.top2000.csv")
saveRDS(cell.mat.split, "Consensus method/Figure 2/Assets/B/cell.mat.split.rds")



B.Pl.hq@meta.data <- B.Pl.hq@meta.data %>% 
  mutate(GP = case_when(
    rownames(B.Pl.hq@meta.data) %in% c(cell.mat.split[["GP1"]]) ~ "GP1",
    rownames(B.Pl.hq@meta.data) %in% c(cell.mat.split[["GP2"]]) ~ "GP2",
    rownames(B.Pl.hq@meta.data) %in% c(cell.mat.split[["GP3"]]) ~ "GP3",
    rownames(B.Pl.hq@meta.data) %in% c(cell.mat.split[["GP4"]]) ~ "GP4",
    rownames(B.Pl.hq@meta.data) %in% c(cell.mat.split[["GP5"]]) ~ "GP5",
    rownames(B.Pl.hq@meta.data) %in% c(cell.mat.split[["GP6"]]) ~ "GP6",
   
    T ~ "unresolved"))

B.Pl.gp_seurat <- subset(B.Pl.hq, cells = cell.mat.top2000$Cell.ID)

saveRDS(B.Pl.gp_seurat, "Consensus method/Figure 2/Assets/B/b.top2000.rds")




DotPlot(B.Pl.gp_seurat, group.by = "GP", features = c("CD3E", "CD3D"))
DotPlot(B.Pl.gp_seurat, group.by = "Anno_Clusters_index", features = names(B.Pl_gps))


B.Pl.hq2 <- B.Pl.hq %>% subset(subset = GP != "unresolved")
...
B.Pl.hq2 <- B.Pl.hq2 %>% RunUMAP(reduction = "scANVI", dims = 1:20, min.dist = 0.5, n.neighbors = 50 )

B.Pl.hq2@reductions$umap@key <- "UMAP_"

colnames(x = B.Pl.hq2[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)


tiff("Consensus method/Figure 2/Assets/B/B.Pl.UMAP.tiff", width = 900, height = 900, res = 300)
DimPlot(B.Pl.hq2, group.by = "GP", cols = gp_cols, label = F,label.size = 5) +
  ggtitle(paste0("B cells (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()


B.Pl.gp_seurat <- B.Pl.gp_seurat %>% RunUMAP(reduction = "scANVI", dims = 1:30, min.dist = 0.2, n.neighbors = 100 )

B.Pl.gp_seurat@reductions$umap@key <- "UMAP_"

colnames(x = B.Pl.gp_seurat[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/B Plasma/B.Pl.UMAP.top2000.tiff", width = 900, height = 900, res = 300)
DimPlot(B.Pl.gp_seurat, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("B cells (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()
...
plot_top3_genes(B.Pl_gps, B.Pl.gp_seurat, "B")

fgsea_res <- read.csv("Consensus method/Figure 2/Assets/B/fgsea.results.csv")
fgsea_res <- fgsea_res %>% 
  group_by(GP) %>% arrange(desc(NES))


B.Pl_gps

B.cell.signatures <- read.csv("Consensus method/Figure 2/Assets/B/B cell signatures.csv", 
                              check.names = F)


gene_signatures <- as.list(B.cell.signatures)

gene_signatures <- lapply(B.cell.signatures, function(x) Filter(function(y) y != "" && !is.null(y), x))




start <- length(colnames(B.Pl.hq2@meta.data)) + 1

B.Pl.hq2 <- AddModuleScore(B.Pl.hq2, features = gene_signatures, name = "GS")

colnames(B.Pl.hq2@meta.data)[start:length(colnames(B.Pl.hq2@meta.data))] <- names(gene_signatures)
colnames(B.Pl.hq2@meta.data)

names(gene_signatures)



DotPlot(B.Pl.hq2,group.by = "GP", features = names(gene_signatures))
...
B_markers <- list(
  `GC` = c("BIRC5", "AURKB", "TACC3", "CKS1B"),
  Memory = c("CRIP1", "CD27", "CCDC50", "TNFRSF13B"),
  Mature = c("MS4A1","CD37",  "BIRC3",  "TCL1A", "IGHD"),
 `Pre-GC` = c("CCND2", "CD83",  "NCL", "FABP5"),
Stress = c("JUN", "FOS", "TNF", "NFKB"),
Cytokine = c("CCL5", "CCL4")
)
...
Idents(B.Pl.gp_seurat) <- "GP"
p_dot <- DotPlot(B.Pl.gp_seurat, group.by = "GP",features = B_markers) +
  scale_y_discrete(limits=rev)+
...
p_dot
library(patchwork)
tiff(paste0("Consensus method/Figure 2/Assets/B/DotPlot.tiff"), width = 3800, height = 1000, res = 300)
p_y + p_dot +
  plot_layout(widths = unit(c(1, 20), "cm"))
dev.off()
...
wilcox.genes <- wilcoxauc(B.Pl.hq2, group_by = "GP")
write.csv(wilcox.genes, "Consensus method/Figure 2/Assets/B/wilcox.results.csv")
...
fwrite(data.frame(gsea.gp.df), "Consensus method/Figure 2/Assets/B/fgsea.results.csv")


gsea.gp.df <- read.csv("Consensus method/Figure 2/Assets/B/fgsea.results.csv")
...
sample.cells <- B.Pl.gp_seurat@meta.data %>% 
...
sample.pdac <- subset(B.Pl.gp_seurat, cells = sample.cells$Cell.ID)

Idents(sample.pdac) <- "GP"

signature.matrix <- GetAssayData(sample.pdac, assay = "RNA", slot = "counts")

signature.matrix <- as.data.frame(signature.matrix)


signature.matrix <- signature.matrix %>% select(sample.cells$Cell.ID)

identical(sample.cells$Cell.ID, colnames(signature.matrix))

colnames(signature.matrix) <- sample.cells$GP

colnames(signature.matrix) <- gsub("\\..*", "", colnames(signature.matrix))

signature.matrix <- signature.matrix %>% 
  filter(rownames(.) %in% consensus_features.df)

write.csv(signature.matrix, "B.signature.matrix.csv")
