# CD4 T cells gene programme discovery
...
cd4t <- subset(lymphoid, subset = Lvl2_Clusters %in% c("CD4T", "CD8T", "Proliferating T/NK"))
cd4t

cd4t.hq <- subset(cd4t, subset = CD8A < 1 & CD8B < 1 & CD3E > 1)
cd4t
cd4t.hq


cd4t.list <- SplitObject(cd4t.hq, split.by = "Study")



# normalize and identify variable features for each dataset independently
cd4t.list <- lapply(X = cd4t.list, FUN = function(x) {
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 5000)
})

# select features that are repeatedly variable across datasets for integration
consensus_features <- SelectIntegrationFeatures(object.list = cd4t.list, nfeatures = 5000)
...
counts <- GetAssayData(cd4t.hq, assay = "RNA")
dim(counts)
...
cd4t.hq.subset <- subset(cd4t.hq, features = rownames(counts))

print(cd4t.hq)
print(cd4t.hq.subset)

rm(cd4t.list);rm(counts);gc()
...
gp_generation(cd4t.hq.subset)

write.csv(gene.programmes, "Consensus method/Figure 2/Assets/CD4T/gene.programmes.csv")
write.csv(jaccard.matrix, "Consensus method/Figure 2/Assets/CD4T/jaccard.matrix.csv")


gene.programmes <- read.csv("Consensus method/Figure 2/Assets/CD4T/gene.programmes.csv", row.names = 1)
jaccard.matrix <- read.csv("Consensus method/Figure 2/Assets/CD4T/jaccard.matrix.csv", row.names = 1)
...
rcc = ConsensusClusterPlus(as.matrix(jaccard.matrix),maxK=10,reps=500,pItem=0.8,pFeature=1,
                           ...
                           title = "Consensus method/Figure 2/Assets/CD4T/ccp_results",
                           distance="pearson",clusterAlg="pam")

saveRDS(rcc, "Consensus method/Figure 2/Assets/CD4T/cd4t.consensus.results.rds")
rcc <- readRDS("Consensus method/Figure 2/Assets/CD4T/cd4t.consensus.results.rds")
...
plot_consensus_heatmap("CD4+ T", "CD4T")
...
cd4t_gps <- split(x = fc_res.top$gene, f = fc_res.top$GP)
...
cd4t_gps <- lapply(cd4t_gps, top_20_genes)

cd4t_gps

optK <- 7
cd4t_gps <- readRDS("Consensus method/Figure 2/Assets/CD4T/cd4t_gps.rds")
# Apply to CD4T cells


colnames(cd4t.hq@meta.data)
cd4t.hq@meta.data <- cd4t.hq@meta.data[,-c(18:length(colnames(cd4t.hq@meta.data)))]

cd4t.hq <- AddModuleScore(cd4t.hq, features = cd4t_gps, name = "GP")


cd4t.gp_mat <- cd4t.hq@meta.data %>% 
  select(names(cd4t_gps)) %>% scale() %>% as.data.frame()


assigned_GPs <- apply(cd4t.gp_mat, 1, function(row) {
...
cd4t.gp_mat$GP <- assigned_GPs


table(cd4t.gp_mat$GP)
colnames(cd4t.gp_mat)

cd4t.gp_mat <- cd4t.gp_mat %>% rownames_to_column(var = "Cell.ID")
max_values <- apply(cd4t.gp_mat[,2:(optK+1)], 1, max)
table(cd4t.gp_mat$GP)

cd4t.gp_mat$Max_Variable <- max_values

cell.mat.top2000 <- cd4t.gp_mat %>%
  group_by(GP) %>% 
...
cell.mat.split <- split(x = cd4t.gp_mat$Cell.ID, cd4t.gp_mat$GP)

saveRDS(cell.mat.top2000, "Consensus method/Figure 2/Assets/CD4T/cell.mat.top2000.csv")
saveRDS(cell.mat.split, "Consensus method/Figure 2/Assets/CD4T/cell.mat.split.rds")



cd4t.hq@meta.data <- cd4t.hq@meta.data %>% 
  mutate(GP = case_when(
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP1"]]) ~ "GP1",
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP2"]]) ~ "GP2",
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP3"]]) ~ "GP3",
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP4"]]) ~ "GP4",
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP5"]]) ~ "GP5",
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP6"]]) ~ "GP6",
    rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP7"]]) ~ "GP7",
 #   rownames(cd4t.hq@meta.data) %in% c(cell.mat.split[["GP8"]]) ~ "GP8",
    T ~ "unresolved"))

cd4t.gp_seurat <- subset(cd4t.hq, cells = cell.mat.top2000$Cell.ID)
cd4t.gp_seurat


saveRDS(cd4t.gp_seurat, "Consensus method/Figure 2/Assets/CD4T/cd4t.top2000.rds")


DotPlot(cd4t.gp_seurat, group.by = "Anno_Clusters_index", features = names(cd4t_gps))


cd4t.hq2 <- cd4t.hq %>% subset(subset = GP != "unresolved")
...
gp.sign <- cd4t.hq2@meta.data %>% 
  rownames_to_column(var = "Cell.ID") %>% 
  select(Study.Patient.ID,names(cd4t_gps),Cell.ID) %>% 
  pivot_longer(cols = names(cd4t_gps), values_to = "ES", names_to = "GP") %>% 
...
gp.sign <- read.csv("Consensus method/Figure 2/Assets/CD4T/cd4t.gp.props.c3.csv", row.names = 1)


write.csv(gp.sign, "Consensus method/Figure 2/Assets/CD4T/cd4t.gp.sign.c3.csv")
...
cd4t.hq2 <- cd4t.hq2 %>% RunUMAP(reduction = "scANVI", dims = 1:30, min.dist = 0.2, n.neighbors = 30 )

cd4t.hq2@reductions$umap@key <- "UMAP_"

colnames(x = cd4t.hq2[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/CD4T/cd4t.UMAP.tiff", width = 900, height = 900, res = 300)
DimPlot(cd4t.hq2, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("CD4+ T (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()

cd4t.gp_seurat <- cd4t.gp_seurat %>% RunUMAP(reduction = "scANVI", dims = 1:30, min.dist = 0.2, n.neighbors = 100 )

cd4t.gp_seurat@reductions$umap@key <- "UMAP_"

colnames(x = cd4t.gp_seurat[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/CD4T/cd4t.UMAP.top2000.tiff", width = 900, height = 900, res = 300)
DimPlot(cd4t.gp_seurat, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("CD4+ T (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()
...
plot_top3_genes(cd4t_gps, cd4t.gp_seurat, "CD4+ T", "CD4T")
...
cd4t_markers <- list(
  Naive = c("CCR7", "TCF7", "SELL"),
  `Central\nMemory` = c("ANXA1", "ANXA2", "IL7R"),
  Treg = c("FOXP3", "IL2RA", "IKZF2"),
  Cytotoxic = c("GNLY", "GZMB", "PRF1"),
  `Effector\nMemory` = c("GZMK", "FOSB", "JUN", "CCL5"),
  Proliferation = c("AURKB", "STMN1", "TOP2A"),
  `Follicular\nHelper` = c("CXCL13", "TOX", "PDCD1")
...
Idents(cd4t.gp_seurat) <- "GP"
p_dot <- DotPlot(cd4t.gp_seurat, group.by = "GP",features = cd4t_markers) +
  scale_y_discrete(limits=rev)+
...
p_dot
library(patchwork)
tiff(paste0("Consensus method/Figure 2/Assets/CD4T/DotPlot.tiff"), width = 3800, height = 1000, res = 300)
p_y + p_dot +
...
wilcox.genes <- wilcoxauc(cd4t.hq2, group_by = "GP")
write.csv(wilcox.genes, "Consensus method/Figure 2/Assets/CD4T/wilcox.results.csv")

wilcox.genes.top20 <- wilcox.genes %>% 
  group_by(group) %>% 
  slice_max(n = 20, order = logFC)
...
fwrite(data.frame(gsea.gp.df), "Consensus method/Figure 2/Assets/CD4T/fgsea.results.csv")
