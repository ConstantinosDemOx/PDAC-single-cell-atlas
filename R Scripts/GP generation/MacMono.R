# Mac/Mono gene programme discovery
...
FeaturePlot(myeloid, features = "MZB1")



mac.mono <- subset(myeloid, subset = Lvl2_Clusters %in% c("Macrophages", "Monocytes"))
mac.mono



mac.mono.hq <- subset(mac.mono, subset = CD3E < 1 & CD3D < 1 & IL7R < 1 & NKG7 < 1 & CD3G < 1)
mac.mono
mac.mono.hq

mac.mono.list <- SplitObject(mac.mono.hq, split.by = "Study")



# normalize and identify variable features for each dataset independently
mac.mono.list <- lapply(X = mac.mono.list, FUN = function(x) {
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 5000)
})

# select features that are repeatedly variable across datasets for integration
consensus_features <- SelectIntegrationFeatures(object.list = mac.mono.list, nfeatures = 5000)
...
counts <- GetAssayData(mac.mono.hq, assay = "RNA")
dim(counts)
...
mac.mono.hq.subset <- subset(mac.mono.hq, features = rownames(counts))

print(mac.mono.hq)
print(mac.mono.hq.subset)

rm(mac.mono.list);rm(counts);gc()
...
gp_generation(mac.mono.hq.subset)
...
write.csv(gene.programmes, "Consensus method/Figure 2/Assets/MacMono/gene.programmes.csv")
write.csv(jaccard.matrix, "Consensus method/Figure 2/Assets/MacMono/jaccard.matrix.csv")

gene.programmes <- read.csv("Consensus method/Figure 2/Assets/MacMono/gene.programmes.csv", row.names = 1)
jaccard.matrix <- read.csv("Consensus method/Figure 2/Assets/MacMono/jaccard.matrix.csv", row.names = 1, check.names = F)



fviz_nbclust(jaccard.matrix, FUN = hcut, method = "wss")


distance_matrix <- as.dist(1- jaccard.matrix)
hc <- hclust(distance_matrix, method = "average")
# Test different numbers of clusters
k_values <- 2:15 # Adjust range as needed
avg_silhouette_widths <- sapply(k_values, silhouette_width)
...
tiff("Consensus method/Figure 3/Assets/Silhouette.plot.tiff", width = 1200, height = 1200, res = 300)
plot(k_values, avg_silhouette_widths, type = "b", pch = 19,
...
rcc = ConsensusClusterPlus(as.matrix(jaccard.matrix),maxK=12,reps=500,pItem=0.8,pFeature=1,
                           seed = 1000,plot="png",
                           innerLinkage = "average", 
                           finalLinkage = "average",
                           title = "Consensus method/Figure 2/Assets/MacMono/ccp_results",
                           distance="pearson",clusterAlg="pam")


saveRDS(rcc, "Consensus method/Figure 2/Assets/MacMono/mac.mono.consensus.results.rds")
rcc <- readRDS("Consensus method/Figure 2/Assets/MacMono/mac.mono.consensus.results.rds")
...
plot_consensus_heatmap("MacMono")
...
macmono_gps <- split(x = fc_res.top$gene, f = fc_res.top$GP)
...
macmono_gps <- lapply(macmono_gps, top_20_genes)

macmono_gps


#saveRDS(macmono_gps, "Consensus method/Figure 2/Assets/MacMono/macmono_gps4.rds")
...
colnames(mac.mono.hq@meta.data)
mac.mono.hq@meta.data <- mac.mono.hq@meta.data[,-c(18:length(colnames(mac.mono.hq@meta.data)))]

mac.mono.hq <- AddModuleScore(mac.mono.hq, features = macmono_gps, name = "GP")


mac.mono.gp_mat <- mac.mono.hq@meta.data %>% 
  select(names(macmono_gps)) %>% scale() %>% as.data.frame()


assigned_GPs <- apply(mac.mono.gp_mat, 1, function(row) {
...
mac.mono.gp_mat$GP <- assigned_GPs


table(mac.mono.gp_mat$GP)
colnames(mac.mono.gp_mat)

mac.mono.gp_mat <- mac.mono.gp_mat %>% rownames_to_column(var = "Cell.ID")
max_values <- apply(mac.mono.gp_mat[,2:(optK+1)], 1, max)
table(mac.mono.gp_mat$GP)

mac.mono.gp_mat$Max_Variable <- max_values

cell.mat.top2000 <- mac.mono.gp_mat %>%
  group_by(GP) %>% 
...
cell.mat.split <- split(x = mac.mono.gp_mat$Cell.ID, mac.mono.gp_mat$GP)


saveRDS(cell.mat.top2000, "Consensus method/Figure 2/Assets/MacMono/cell.mat.top2000.csv")
saveRDS(cell.mat.split, "Consensus method/Figure 2/Assets/MacMono/cell.mat.split.rds")


mac.mono.hq@meta.data <- mac.mono.hq@meta.data %>% 
  mutate(GP = case_when(
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP1"]]) ~ "GP1",
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP2"]]) ~ "GP2",
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP3"]]) ~ "GP3",
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP4"]]) ~ "GP4",
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP5"]]) ~ "GP5",
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP6"]]) ~ "GP6",
    rownames(mac.mono.hq@meta.data) %in% c(cell.mat.split[["GP7"]]) ~ "GP7",
    T ~ "unresolved"))

mac.mono.gp_seurat <- subset(mac.mono.hq, cells = cell.mat.top2000$Cell.ID)

mac.mono.gp_seurat

saveRDS(mac.mono.gp_seurat, "Consensus method/Figure 2/Assets/MacMono/macmono.top2000.rds")




DotPlot(mac.mono.gp_seurat, group.by = "GP", features = c("SPP1", "C1QB", "C1QA","APOE", "CCL18", "IL1A", "FCN1", "FCGR3A", "MMP9", "STMN1"))
DotPlot(mac.mono.gp_seurat, group.by = "Anno_Clusters_index", features = names(macmono_gps))


mac.mono.hq2 <- mac.mono.hq %>% subset(subset = GP != "unresolved")
...
mac.mono.hq2 <- mac.mono.hq2 %>% RunUMAP(reduction = "scANVI", dims = 1:30, min.dist = 0.6 )

mac.mono.hq2@reductions$umap@key <- "UMAP_"

colnames(x = mac.mono.hq2[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/MacMono/mac.mono.UMAP.tiff", width = 900, height = 900, res = 300)
DimPlot(mac.mono.hq2, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("MacMono (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()

mac.mono.gp_seurat <- mac.mono.gp_seurat %>% RunUMAP(reduction = "scANVI", dims = 1:30, min.dist = 0.2, n.neighbors = 100 )

mac.mono.gp_seurat@reductions$umap@key <- "UMAP_"

colnames(x = mac.mono.gp_seurat[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/MacMono/mac.mono.UMAP.top2000.tiff", width = 900, height = 900, res = 300)
DimPlot(mac.mono.gp_seurat, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("MacMono (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()
...
plot_top3_genes(macmono_gps, mac.mono.gp_seurat, "MacMono")


fgsea.results <- read.csv("Consensus method/Figure 2/Assets/MacMono/fgsea.results.csv")
...
MacMono_markers <- list(
  `Antigen\nPresentation` = c("HLA-DQA1", "HLA-DOA", "HLA-DPB1"),
 #Complement = c("C1QA", "C1QB", "C1QC"),
  Monocyte = c("FCN1",  "S100A9"),
  `Lipid-associated` = c("APOE", "APOC1", "PLA2G7", "CCL18"),
  Hypoxia = c("ENO2", "HK2",  "SLC2A1"),
  `M1` = c("TNF", "IL1B"),
  `M2` = c("MRC1", "MSR1", "CD206", "CD163"),
  Stress = c("DUSP2", "NFKBIA","FOSB"),
 Apoptosis = c("CD44", "ANXA1", "BCL2A1"),
...
Idents(mac.mono.gp_seurat) <- "GP"
p_dot <- DotPlot(mac.mono.gp_seurat, group.by = "GP",features = MacMono_markers) +
  scale_y_discrete(limits=rev)+
...
p_dot
library(patchwork)
tiff(paste0("Consensus method/Figure 2/Assets/MacMono/DotPlot.tiff"), width = 3800, height = 1000, res = 300)
p_y + p_dot +
  plot_layout(widths = unit(c(1, 25), "cm"))
dev.off()
...
Idents(mac.mono.hq2) <- "GP"

wilcox.genes <- wilcoxauc(mac.mono.hq2, group_by = "GP")
write.csv(wilcox.genes, "Consensus method/Figure 2/Assets/MacMono/wilcox.results.csv")
wilcox.genes <- read.csv("Consensus method/Figure 2/Assets/MacMono/wilcox.results.csv")
...
fwrite(data.frame(gsea.gp.df), "Consensus method/Figure 2/Assets/MacMono/fgsea.results.csv")
gsea.gp.df <- do.call(rbind, gsea.gp.list)
