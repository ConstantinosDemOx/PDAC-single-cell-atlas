# Epithelial gene programme discovery
...
pdac.atlas@meta.data <- pdac.atlas@meta.data %>% 
  mutate(GP = case_when(
    ...
    str_detect(Lvl2_Clusters, "Monocyte|Macrophage|DC|ductalrophil") ~ "Myeloid",
    str_detect(Lvl2_Clusters, "Ductal|Acinar|Hepatocyte") ~ "Epithelial\n(non-malignant)",
    str_detect(Lvl2_Clusters, "Malignant|Tuft") ~ "Epithelial\n(malignant)",
    TRUE ~ Lvl2_Clusters))

pdac.atlas@meta.data <- pdac.atlas@meta.data %>% 
  mutate(GP = factor(GP, levels = c(
    "Epithelial\n(malignant)", "Epithelial\n(non-malignant)", "Endocrine", "Endothelial",
    "Stellate", "Fibroblast", "Schwann", "Myeloid", "Mast", "Plasma", "B", "T/NK"
  )))


epi <- subset(pdac.atlas, subset = GP %in% c("Epithelial\n(malignant)", "Epithelial\n(non-malignant)"))

rm(pdac.atlas);gc()



epi.hq <- subset(epi, subset = CD3D < 1 &  PTPRC < 1 & IL7R < 1 & CD3E < 1 & CD68 < 1)
epi.hq

rm(epi);gc()


epi.list <- SplitObject(epi.hq, split.by = "Study")
...
consensus_features <- SelectIntegrationFeatures(object.list = epi.list, nfeatures = 5000)
...
counts <- GetAssayData(epi.hq, assay = "RNA")
dim(counts)
...
epi.hq.subset <- subset(epi.hq, features = rownames(counts))

print(epi.hq)
print(epi.hq.subset)

rm(epi.list);rm(counts);gc()
...
gp_generation(epi.hq.subset)

write.csv(gene.programmes, "Consensus method/Figure 2/Assets/Epithelial/gene.programmes.csv")
write.csv(jaccard.matrix, "Consensus method/Figure 2/Assets/Epithelial/jaccard.matrix.csv")
gene.programmes <- read.csv("Consensus method/Figure 2/Assets/Epithelial/gene.programmes.csv")
...
rcc = ConsensusClusterPlus(as.matrix(jaccard.matrix),maxK=12,reps=500,pItem=0.8,pFeature=1,
                           ...
                           title = "Consensus method/Figure 2/Assets/Epithelial/ccp_results",
                           distance="pearson",clusterAlg="pam")

#saveRDS(rcc, "Consensus method/Figure 2/Assets/Epithelial/epi.consensus.results.rds")
rcc <- readRDS("Consensus method/Figure 2/Assets/Epithelial/epi.consensus.results.rds")
...
plot_consensus_heatmap("Epithelial")





gene.programmes <- read.csv("Consensus method/Figure 2/Assets/Epithelial/gene.programmes.csv", row.names = 1)
...
epi_gps <- split(x = fc_res.top$gene, f = fc_res.top$GP)
...
epi_gps <- lapply(epi_gps, top_20_genes)

epi_gps

saveRDS(epi_gps, "Consensus method/Figure 2/Assets/Epithelial/epi_gps.rds")
epi_gps <- readRDS("Consensus method/Figure 2/Assets/Epithelial/epi_gps.rds")
# Apply to epithelial cells


colnames(epi.hq@meta.data)
epi.hq@meta.data <- epi.hq@meta.data[,-c(17:length(colnames(epi.hq@meta.data)))]

epi.hq <- AddModuleScore(epi.hq, features = epi_gps, name = "GP")


epi.gp_mat <- epi.hq@meta.data %>% 
  select(names(epi_gps)) %>% scale() %>% as.data.frame()


assigned_GPs <- apply(epi.gp_mat, 1, function(row) {
...
epi.gp_mat$GP <- assigned_GPs


table(epi.gp_mat$GP)
colnames(epi.gp_mat)

epi.gp_mat <- epi.gp_mat %>% rownames_to_column(var = "Cell.ID")
max_values <- apply(epi.gp_mat[,2:(optK+1)], 1, max)
table(epi.gp_mat$GP)

epi.gp_mat$Max_Variable <- max_values

cell.mat.top2000 <- epi.gp_mat %>%
  group_by(GP) %>% 
  slice_max(n = 2000, order_by = Max_Variable) %>% 
  filter(GP != "unresolved")

saveRDS(cell.mat.top2000, "Consensus method/Figure 2/Assets/Epithelial/cell.mat.top2000.csv")

table(cell.mat.top2000$GP)



cell.mat.split <- split(x = epi.gp_mat$Cell.ID, epi.gp_mat$GP)
saveRDS(cell.mat.split, "Consensus method/Figure 2/Assets/Epithelial/cell.mat.split.rds")



epi.hq@meta.data <- epi.hq@meta.data %>% 
  mutate(GP = case_when(
    rownames(epi.hq@meta.data) %in% c(cell.mat.split[["GP1"]]) ~ "GP1",
    rownames(epi.hq@meta.data) %in% c(cell.mat.split[["GP2"]]) ~ "GP2",
    rownames(epi.hq@meta.data) %in% c(cell.mat.split[["GP3"]]) ~ "GP3",
    rownames(epi.hq@meta.data) %in% c(cell.mat.split[["GP4"]]) ~ "GP4",
    rownames(epi.hq@meta.data) %in% c(cell.mat.split[["GP5"]]) ~ "GP5",
    rownames(epi.hq@meta.data) %in% c(cell.mat.split[["GP6"]]) ~ "GP6",
    T ~ "unresolved"))

epi.gp_seurat <- subset(epi.hq, cells = cell.mat.top2000$Cell.ID)

saveRDS(epi.gp_seurat, "Consensus method/Figure 2/Assets/Epithelial/epi.top2000.rds")

epi.hq2 <- epi.hq %>% subset(subset = GP != "unresolved")
...
epi.hq2 <- epi.hq2 %>% RunUMAP(reduction = "scANVI", dims = 1:10)

epi.hq2@reductions$umap@key <- "UMAP_"

colnames(x = epi.hq2[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/Epithelial/epi.UMAP.tiff", width = 900, height = 900, res = 300)
DimPlot(epi.hq2, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("Epithelial (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()



epi.gp_seurat <- readRDS("Consensus method/Figure 2/Assets/Epithelial/epi.top2000.rds")
epi_gps <- readRDS("Consensus method/Figure 2/Assets/Epithelial/epi_gps.rds")
optK <- 6
...
plot_top3_genes(epi_gps, epi.gp_seurat, "Epithelial")
...
epi_markers <- list(
  ...
  `Basal-like` = c("KRT7", "KRT17",  "LY6D"),
  `Classical-like` = c("TFF1", "TFF3", "CEACAM6", "GATA6"),
  `Antigen\nPresentation` = c("HLA-DRB1", "HLA-DPA1", "CD74"),
  Acinar = c("CEL", "CELA3A", "CLPS"),
  `Ductal` = c("SLC4A4", "CFTR"),
 
  Proliferation = c("STMN1", "MKI67", "TOP2A")
)
...
Idents(epi.gp_seurat) <- "GP"
p_dot <- DotPlot(epi.gp_seurat, group.by = "GP",features = epi_markers) +
  scale_y_discrete(limits=rev)+
...
library(patchwork)
tiff(paste0("Consensus method/Figure 2/Assets/Epithelial/DotPlot.tiff"), width = 3500, height = 1000, res = 300)
p_y + p_dot +
  plot_layout(widths = unit(c(1, 18), "cm"))
dev.off()
...
gene_signatures <- read.csv("New way/Figure_2/Files/gene signatures/malignant_gene_signatures.csv", check.names = F)

gene_signatures <- gene_signatures %>%  select(!matches("Stroma"))
...
wilcox.genes <- wilcoxauc(epi.gp_seurat, group_by = "GP")
write.csv(wilcox.genes, "Consensus method/Figure 2/Assets/Epithelial/wilcox.results.csv")

wilcox.results <- read.csv("Consensus method/Figure 2/Assets/Epithelial/wilcox.results.csv", row.names = 1)
...
fwrite(data.frame(gsea.gp.df), "Consensus method/Figure 2/Assets/Epithelial/fgsea.results.csv")

gsea.gp.df <- read.csv("Consensus method/Figure 2/Assets/Epithelial/fgsea.results.csv")
...
tiff("Consensus method/Figure 2/Assets/GP6.GSEA.tiff", width = 1200, height = 900, res = 300)
ggbarplot(gsea.gp6, "pathway", "NES",
  ...
  ggtitle("Epithelial GP6 (FDR < 0.05)")+
  labs(fill="")+
