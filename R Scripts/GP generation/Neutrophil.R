...
neut <- subset(myeloid, subset = Lvl2_Clusters %in% c("Neutrophil"))





neut.hq <- subset(neut, subset = CD3E < 1 & CD3D < 1)
neut
neut.hq

neut.list <- SplitObject(neut.hq, split.by = "Study")



# normalize and identify variable features for each dataset independently
neut.list <- lapply(X = neut.list, FUN = function(x) {
  x <- NormalizeData(x)
  x <- FindVariableFeatures(x, selection.method = "vst", nfeatures = 5000)
})

# select features that are repeatedly variable across datasets for integration
consensus_features <- SelectIntegrationFeatures(object.list = neut.list, nfeatures = 5000)
...
counts <- GetAssayData(neut.hq, assay = "RNA")
dim(counts)
...
neut.hq.subset <- subset(neut.hq, features = rownames(counts))

print(neut.hq)
print(neut.hq.subset)

rm(neut.list);rm(counts);gc()
...
write.csv(gene.programmes, "Consensus method/Figure 2/Assets/Neutrophil/gene.programmes.csv")
write.csv(jaccard.matrix, "Consensus method/Figure 2/Assets/Neutrophil/jaccard.matrix.csv")
gene.programmes <- read.csv("Consensus method/Figure 2/Assets/Neutrophil/gene.programmes.csv", row.names = 1)
jaccard.matrix <- read.csv("Consensus method/Figure 2/Assets/Neutrophil/jaccard.matrix.csv", row.names = 1)



library(ConsensusClusterPlus)

rcc = ConsensusClusterPlus(as.matrix(jaccard.matrix),maxK=8,reps=500,pItem=0.8,pFeature=1,
                           seed = 1000,plot="png",
                           innerLinkage = "average", 
                           finalLinkage = "average",
                           title = "Consensus method/Figure 2/Assets/Neutrophil/ccp_results",
                           distance="pearson",clusterAlg="pam")


saveRDS(rcc, "Consensus method/Figure 2/Assets/Neutrophil/neut.consensus.results.rds")
rcc <- readRDS("Consensus method/Figure 2/Assets/Neutrophil/neut.consensus.results.rds")
...
plot_consensus_heatmap("Neutrophil")
...
Neutrophil_gps <- split(x = fc_res.top$gene, f = fc_res.top$GP)
...
Neutrophil_gps <- lapply(Neutrophil_gps, top_20_genes)

Neutrophil_gps
saveRDS(Neutrophil_gps, "Consensus method/Figure 2/Assets/Neutrophil/Neutrophil_gps.rds")
Neutrophil_gps <- readRDS("Consensus method/Figure 2/Assets/Neutrophil/Neutrophil_gps.rds")
# Apply to Neutrophils


colnames(neut.hq@meta.data)
neut.hq@meta.data <- neut.hq@meta.data[,-c(18:length(colnames(neut.hq@meta.data)))]

neut.hq <- AddModuleScore(neut.hq, features = Neutrophil_gps, name = "GP")


neut.gp_mat <- neut.hq@meta.data %>% 
  select(names(Neutrophil_gps)) %>% scale() %>% as.data.frame()


assigned_GPs <- apply(neut.gp_mat, 1, function(row) {
...
neut.gp_mat$GP <- assigned_GPs


table(neut.gp_mat$GP)
colnames(neut.gp_mat)

neut.gp_mat <- neut.gp_mat %>% rownames_to_column(var = "Cell.ID")
max_values <- apply(neut.gp_mat[,2:(optK+1)], 1, max)
table(neut.gp_mat$GP)

neut.gp_mat$Max_Variable <- max_values

cell.mat.top2000 <- neut.gp_mat %>%
  group_by(GP) %>% 
...
cell.mat.split <- split(x = neut.gp_mat$Cell.ID, neut.gp_mat$GP)


saveRDS(cell.mat.top2000, "Consensus method/Figure 2/Assets/Neutrophil/cell.mat.top2000.csv")
saveRDS(cell.mat.split, "Consensus method/Figure 2/Assets/Neutrophil/cell.mat.split.rds")
...
neut.hq@meta.data <- neut.hq@meta.data %>% 
  mutate(GP = case_when(
    rownames(neut.hq@meta.data) %in% c(cell.mat.split[["GP1"]]) ~ "GP1",
    rownames(neut.hq@meta.data) %in% c(cell.mat.split[["GP2"]]) ~ "GP2",
    rownames(neut.hq@meta.data) %in% c(cell.mat.split[["GP3"]]) ~ "GP3",
    T ~ "unresolved"))

neut.gp_seurat <- subset(neut.hq, cells = cell.mat.top2000$Cell.ID)

saveRDS(neut.gp_seurat, "Consensus method/Figure 2/Assets/Neutrophil/neut.top2000.rds")





DotPlot(neut.gp_seurat, group.by = "GP", features = c("SPP1", "C1QB", "C1QA","APOE", "CCL18", "IL1A", "FCN1", "FCGR3A", "MMP9", "STMN1"))
DotPlot(neut.gp_seurat, group.by = "Anno_Clusters_index", features = names(Neutrophil_gps))


neut.hq2 <- neut.hq %>% subset(subset = GP != "unresolved")
...
neut.hq2 <- neut.hq2 %>% RunUMAP(reduction = "scANVI", dims = 1:20, min.dist = 0.6, n.neighbors = 10 )

neut.hq2@reductions$umap@key <- "UMAP_"

colnames(x = neut.hq2[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/Neutrophil/neut.UMAP.tiff", width = 900, height = 900, res = 300)
DimPlot(neut.hq2, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("Neutrophil (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()

neut.gp_seurat <- neut.gp_seurat %>% RunUMAP(reduction = "scANVI", dims = 1:20, min.dist = 0.2, n.neighbors = 100 )

neut.gp_seurat@reductions$umap@key <- "UMAP_"

colnames(x = neut.gp_seurat[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/Neutrophil/neut.UMAP.top2000.tiff", width = 900, height = 900, res = 300)
DimPlot(neut.gp_seurat, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("Neutrophil (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()
...
plot_top3_genes(Neutrophil_gps, neut.gp_seurat, "Neutrophil")




fgsea.results <- read.csv("Consensus method/Figure 2/Assets/Neutrophil/fgsea.results.csv")
...
Neutrophil_markers <- list(
  `Neutrophil\nDegranulation` = c("SPI1", "ITGB2", "ITGAM", "ANXA3"),
  Hypoxia = c("CXCR4", "MIF", "VEGFA", "ENO1"),
  `IFN Response` = c("IFIT3", "IFIT2", "IFI16", "STAT1")
...
Idents(neut.gp_seurat) <- "GP"
p_dot <- DotPlot(neut.gp_seurat, group.by = "GP",features = Neutrophil_markers) +
  scale_y_discrete(limits=rev)+
...
p_dot
library(patchwork)
tiff(paste0("Consensus method/Figure 2/Assets/Neutrophil/DotPlot.tiff"), width = 3800, height = 1000, res = 300)
p_y + p_dot +
...
wilcox.genes <- wilcoxauc(neut.hq2, group_by = "GP")
write.csv(wilcox.genes, "Consensus method/Figure 2/Assets/Neutrophil/wilcox.results.csv")
...
fwrite(data.frame(gsea.gp.df), "Consensus method/Figure 2/Assets/Neutrophil/fgsea.results.csv")
