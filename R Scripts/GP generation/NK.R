# NK cells gene programme discovery
...
NK <- subset(lymphoid, subset = Lvl2_Clusters %in% c("NK",  "Proliferating T/NK"))
NK

NK.hq <-  subset(NK, subset = NKG7  > 1)
NK
NK.hq


NK.list <- SplitObject(NK.hq, split.by = "Study")
...
consensus_features <- SelectIntegrationFeatures(object.list = NK.list, nfeatures = 5000)
...
counts <- GetAssayData(NK.hq, assay = "RNA")
dim(counts)
...
NK.hq.subset <- subset(NK.hq, features = rownames(counts))

print(NK.hq)
print(NK.hq.subset)

rm(NK.list);rm(counts);gc()
...
gp_generation(NK.hq.subset)

write.csv(gene.programmes, "Consensus method/Figure 2/Assets/NK/gene.programmes.csv")
write.csv(jaccard.matrix, "Consensus method/Figure 2/Assets/NK/jaccard.matrix.csv")


gene.programmes <- read.csv("Consensus method/Figure 2/Assets/NK/gene.programmes.csv", row.names = 1)
#jaccard.matrix <- read.csv("Consensus method/Figure 2/Assets/NK/jaccard.matrix.csv", row.names = 1)
...
rcc = ConsensusClusterPlus(as.matrix(jaccard.matrix),maxK=10,reps=500,pItem=0.8,pFeature=1,
                           ...
                           title = "Consensus method/Figure 2/Assets/NK/ccp_results",
                           distance="pearson",clusterAlg="pam")

saveRDS(rcc, "Consensus method/Figure 2/Assets/NK/NK.consensus.results.rds")
rcc <- readRDS("Consensus method/Figure 2/Assets/NK/NK.consensus.results.rds")
...
NK_gps <- split(x = fc_res.top$gene, f = fc_res.top$GP)
...
NK_gps <- lapply(NK_gps, top_20_genes)

NK_gps
saveRDS(NK_gps, "Consensus method/Figure 2/Assets/NK/NK_gps.rds")
optK <- 3
NK_gps <- readRDS("Consensus method/Figure 2/Assets/NK/NK_gps.rds")
# Apply to NK cells


colnames(NK.hq@meta.data)
NK.hq@meta.data <- NK.hq@meta.data[,-c(18:length(colnames(NK.hq@meta.data)))]

NK.hq <- AddModuleScore(NK.hq, features = NK_gps, name = "GP")


NK.gp_mat <- NK.hq@meta.data %>% 
  select(names(NK_gps)) %>% scale() %>% as.data.frame()


assigned_GPs <- apply(NK.gp_mat, 1, function(row) {
...
NK.gp_mat$GP <- assigned_GPs


table(NK.gp_mat$GP)
colnames(NK.gp_mat)

NK.gp_mat <- NK.gp_mat %>% rownames_to_column(var = "Cell.ID")
max_values <- apply(NK.gp_mat[,2:(optK+1)], 1, max)
table(NK.gp_mat$GP)

NK.gp_mat$Max_Variable <- max_values

cell.mat.top2000 <- NK.gp_mat %>%
  group_by(GP) %>% 
...
cell.mat.split <- split(x = NK.gp_mat$Cell.ID, NK.gp_mat$GP)


saveRDS(cell.mat.top2000, "Consensus method/Figure 2/Assets/NK/cell.mat.top2000.csv")
saveRDS(cell.mat.split, "Consensus method/Figure 2/Assets/NK/cell.mat.split.rds")
...
NK.hq@meta.data <- NK.hq@meta.data %>% 
  mutate(GP = case_when(
    rownames(NK.hq@meta.data) %in% c(cell.mat.split[["GP1"]]) ~ "GP1",
    rownames(NK.hq@meta.data) %in% c(cell.mat.split[["GP2"]]) ~ "GP2",
    rownames(NK.hq@meta.data) %in% c(cell.mat.split[["GP3"]]) ~ "GP3",
    T ~ "unresolved"))

NK.gp_seurat <- subset(NK.hq, cells = cell.mat.top2000$Cell.ID)

saveRDS(NK.gp_seurat, "Consensus method/Figure 2/Assets/NK/nk.top2000.rds")
...
DotPlot(NK.gp_seurat, group.by = "GP", features = c("ACKR1", "FLT4", "SEMA3G", "CCL23", "CCL19", "CCL21"))
DotPlot(NK.gp_seurat, group.by = "Anno_Clusters_index", features = names(NK_gps))


NK.hq2 <- NK.hq %>% subset(subset = GP != "unresolved")
...
NK.hq2 <- NK.hq2 %>% RunUMAP(reduction = "scANVI", dims = 1:10, min.dist = 0.2, n.neighbors = 30 )

NK.hq2@reductions$umap@key <- "UMAP_"

colnames(x = NK.hq2[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/NK/NK.UMAP.tiff", width = 900, height = 900, res = 300)
DimPlot(NK.hq2, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("NK (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()

NK.gp_seurat <- NK.gp_seurat %>% RunUMAP(reduction = "scANVI", dims = 1:20, min.dist = 0.2, n.neighbors = 100 )

NK.gp_seurat@reductions$umap@key <- "UMAP_"

colnames(x = NK.gp_seurat[["umap"]]@cell.embeddings) <- paste0("UMAP_", 1:2)

tiff("Consensus method/Figure 2/Assets/NK/NK.UMAP.top2000.tiff", width = 900, height = 900, res = 300)
DimPlot(NK.gp_seurat, group.by = "GP", cols = gp_cols, label.size = 5) +
  ggtitle(paste0("NK (n = ", optK, ")" )) +
  theme(aspect.ratio = 1) + NoLegend()
dev.off()
...
plot_top3_genes(NK_gps, NK.gp_seurat, "NK")
...
NK_markers <- list(
  Cytokines = c("XCL1", "XCL2", "CCL5"),
  Resident = c("ITGA1", "KLRC1", "KLRC2"),
...
Idents(NK.gp_seurat) <- "GP"
p_dot <- DotPlot(NK.gp_seurat, group.by = "GP",features = NK_markers) +
  scale_y_discrete(limits=rev)+
...
p_dot
library(patchwork)
tiff(paste0("Consensus method/Figure 2/Assets/NK/DotPlot.tiff"), width = 3800, height = 1000, res = 300)
p_y + p_dot +
  plot_layout(widths = unit(c(1, 12), "cm"))
dev.off()
...
wilcox.genes <- wilcoxauc(NK.hq2, group_by = "GP")
write.csv(wilcox.genes, "Consensus method/Figure 2/Assets/NK/wilcox.results.csv")
...
fwrite(data.frame(gsea.gp.df), "Consensus method/Figure 2/Assets/NK/fgsea.results.csv")



gsea.gp.df <- read.csv("Consensus method/Figure 2/Assets/NK/fgsea.results.csv")
...
gsea.gp1 <- gsea.gp.df %>% 
  ...
  filter(str_detect(pathway, "Tnfa|Cytokine|Adhesion|Cytoplasmic translation|Lymphocyte activation")) %>% 
  mutate(word_count = str_count(pathway, "\\S+")) %>%
  filter(word_count <= 5)

tiff("Consensus method/Figure 2/Assets/NK.GP1.GSEA.tiff", width = 1400, height = 900, res = 300)
ggbarplot(gsea.gp1, "pathway", "NES",
